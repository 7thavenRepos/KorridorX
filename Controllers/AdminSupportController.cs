...
public sealed class AdminSupportController : ControllerBase
{
    ...
    public async Task<IActionResult> GetTicket(
        ...
        Ok(ApiResponses.Ok(
            await _service.GetTicketAsync(ticketId, ct),
            "Support ticket retrieved successfully."));

    [HttpPost("tickets/{ticketId:guid}/messages")]
    public async Task<IActionResult> Reply(
        Guid ticketId,
        [FromBody] AdminAddSupportMessageRequestDto request,
        CancellationToken ct) =>
        Ok(ApiResponses.Ok(
            await _service.ReplyAsync(GetUserId(), ticketId, request, ct),
    ...
    public async Task<IActionResult> UpdateTicket(
        Guid ticketId,
        [FromBody] UpdateSupportTicketRequestDto request,
        CancellationToken ct) =>
        Ok(ApiResponses.Ok(
            await _service.UpdateTicketAsync(
                GetUserId(),
                ticketId,
                request,
                ct),
    ...
    public async Task<IActionResult> GetDisputes(
        [FromQuery] TransferDisputeStatus? status,
        ...
        CancellationToken ct = default)
    {
        ...
        return Ok(ApiResponses.OkPaged(
            result.Items,
            result.Meta,
            "Transfer disputes retrieved successfully."));
    }
    ...
    public async Task<IActionResult> GetDispute(
        ...
        CancellationToken ct) =>
        Ok(ApiResponses.Ok(
            await _service.GetDisputeAsync(disputeId, ct),
            "Transfer dispute retrieved successfully."));

    [HttpPost("disputes/{disputeId:guid}/resolve")]
    public async Task<IActionResult> ResolveDispute(
        Guid disputeId,
        [FromBody] ResolveTransferDisputeRequestDto request,
        CancellationToken ct) =>
        Ok(ApiResponses.Ok(
            await _service.ResolveDisputeAsync(
                ...
                request,
                ct),
            "Transfer dispute updated successfully."));

    [HttpPost("transfers/{transferId:guid}/investigations")]
    public async Task<IActionResult> CreateInvestigation(
        Guid transferId,
        [FromBody] CreateTransferInvestigationRequestDto request,
        CancellationToken ct) =>
        Ok(ApiResponses.Ok(
            await _service.CreateInvestigationAsync(
                GetUserId(),
                transferId,
                request,
                ct),
            "Transfer investigation opened successfully."));

    [HttpGet("investigations")]
    public async Task<IActionResult> GetInvestigations(
        [FromQuery] TransferInvestigationStatus? status,
        [FromQuery] Guid? assignedToUserId,
        [FromQuery] Guid? transferId,
        ...
        CancellationToken ct = default)
    {
        var result = await _service.GetInvestigationsAsync(
            status,
            assignedToUserId,
            transferId,
            overdue,
        ...
        return Ok(ApiResponses.OkPaged(
            result.Items,
            result.Meta,
            "Transfer investigations retrieved successfully."));
    }

    [HttpGet("investigations/{investigationId:guid}")]
    public async Task<IActionResult> GetInvestigation(
        ...
        Ok(ApiResponses.Ok(
            await _service.GetInvestigationAsync(
                investigationId,
                ct),
            "Transfer investigation retrieved successfully."));

    [HttpPut("investigations/{investigationId:guid}")]
    public async Task<IActionResult> UpdateInvestigation(
        Guid investigationId,
        [FromBody] UpdateTransferInvestigationRequestDto request,
        CancellationToken ct) =>
        Ok(ApiResponses.Ok(
            await _service.UpdateInvestigationAsync(
                GetUserId(),
                investigationId,
                request,
                ct),
            "Transfer investigation updated successfully."));

    [HttpPost("investigations/{investigationId:guid}/evidence")]
    public async Task<IActionResult> AddInvestigationEvidence(
        Guid investigationId,
        [FromBody] AddSupportEvidenceRequestDto request,
        CancellationToken ct) =>
        Ok(ApiResponses.Ok(
            await _service.AddInvestigationEvidenceAsync(
                GetUserId(),
                investigationId,
                request,
                ct),
            "Investigation evidence added successfully."));

    [HttpGet("summary")]
    public async Task<IActionResult> GetSummary(
        CancellationToken ct) =>
        Ok(ApiResponses.Ok(
            await _service.GetSummaryAsync(ct),
            "Support operations summary retrieved successfully."));

    [HttpPost("sla/process")]
    public async Task<IActionResult> ProcessSla(
        [FromBody] ProcessSupportSlaRequestDto request,
        CancellationToken ct = default) =>
        Ok(ApiResponses.Ok(
            new
            {
                Processed = await _service.ProcessSlaBreachesAsync(
                    request.BatchSize,
                    GetUserId(),
                    request.Reason,
                    ct)
            },
            "Support SLA processing completed successfully."));
